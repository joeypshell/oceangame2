...
func capture_and_quit(capture_dir: String) -> void:
	if _main._world == null or _main._player == null:
		push_error("Pass 23 next-dive objective capture requires a loaded playable map.")
		_main.get_tree().quit(1)
		return
	if _main._world.map_id != "production_slice_01":
		push_error("Pass 23 next-dive objective capture loaded unexpected map: %s." % _main._world.map_id)
		_main.get_tree().quit(1)
		return
	...
	if objective.is_empty() or required_targets.is_empty():
		push_error("Pass 23 next-dive objective capture requires primary objective metadata.")
		_main.get_tree().quit(1)
		return
	...
	for target_id in required_targets:
		...
		if target.is_empty():
			push_error("Pass 23 next-dive objective capture missing required target %s." % target_id)
			_main.get_tree().quit(1)
			return
	...
	if not _main._run_complete or result_text.find(EXPECTED_RESULT_COMPLETE) == -1 or result_text.find(EXPECTED_LABEL) == -1:
		push_error("Pass 23 next-dive objective capture expected completed prompt result before saving: %s" % result_text)
		_main.get_tree().quit(1)
		return

	var camera := Camera2D.new()
	camera.name = "Pass23NextDiveObjectiveCaptureCamera"
	...
	var output_path := "%s/%s_pass_23_next_dive_objective.png" % [capture_dir, _safe_filename(_main._world.map_id)]
	var image: Image = _main.get_viewport().get_texture().get_image()
	image.save_png(output_path)
	print("Saved Pass 23 next-dive objective capture: %s" % ProjectSettings.globalize_path(output_path))
	_main.get_tree().quit()
