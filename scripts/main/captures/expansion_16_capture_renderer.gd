...
func _init(main_node) -> void:
	...
	_camera.name = "Expansion16DeeperWreckCaptureCamera"
	_camera.position_smoothing_enabled = false
	_main.add_child(_camera)
	_camera.make_current()
	_mobile_controls = MobileTestControls.new()
	_mobile_controls.name = "Expansion16CaptureMobileControls"
...
func _verify_layout(touch_visible: bool) -> bool:
	...
	for touch_rect in touch_rects:
		if cargo_rect.intersects(touch_rect) or tool_rect.intersects(touch_rect) or status_rect.intersects(touch_rect):
...
func _save_capture(capture_dir: String, filename: String, image: Image) -> bool:
	...
	if error != OK:
		...
		return false
	print("Saved Expansion 16 capture: %s" % ProjectSettings.globalize_path(output_path))
	return true
