...
const PLAYER_OFFSET_TILES := Vector2(-2, 0)
...
func capture_and_quit(capture_dir: String) -> void:
	...
	if segment.is_empty() or target.is_empty():
		push_error("Pass 08 route-extension capture requires segment %s and target %s." % [SEGMENT_ID, TARGET_ID])
		_main.get_tree().quit(1)
		return

	var target_center: Vector2 = target["center"]
	var player_position := target_center + PLAYER_OFFSET_TILES * float(_main._world.tile_size)
	...
	if _main._world.is_salvage_collected(TARGET_ID):
		push_error("Pass 08 route-extension capture setup collected %s; move the review position farther away." % TARGET_ID)
		_main.get_tree().quit(1)
		return
