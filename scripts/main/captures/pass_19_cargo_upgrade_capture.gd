...
const EXPECTED_UPGRADE_FEEDBACK := "Cargo +1 upgraded"
...
func capture_and_quit(capture_dir: String) -> void:
	if _main._world == null or _main._player == null:
		push_error("Pass 19 cargo upgrade capture requires a loaded playable map.")
		_main.get_tree().quit(1)
		return
	if _main._world.map_id != "production_slice_01":
		push_error("Pass 19 cargo upgrade capture loaded unexpected map: %s." % _main._world.map_id)
		_main.get_tree().quit(1)
		return

	_main._hazard_interactions_enabled = false
	var banked_target_ids := _bank_until_upgrade_affordable()
	if banked_target_ids.is_empty() or _main._session_wallet() < _cargo_upgrade_cost():
		push_error("Pass 19 cargo upgrade capture could not bank enough wallet: wallet=%d targets=%s." % [_main._session_wallet(), banked_target_ids])
		_main.get_tree().quit(1)
		return
	...
	if not _main._try_purchase_cargo_capacity_upgrade():
		push_error("Pass 19 cargo upgrade capture could not purchase cargo upgrade: wallet=%d." % wallet_before_purchase)
		_main.get_tree().quit(1)
		return
	...
	var expected_capacity_text := "Held 0/%d" % _main._held_salvage_capacity()
	if status_text.find(EXPECTED_UPGRADE_FEEDBACK) == -1 or status_text.find("Cargo +1") == -1 or status_text.find(expected_capacity_text) == -1:
		push_error("Pass 19 cargo upgrade capture expected wallet, capacity, and upgrade feedback before saving: %s" % status_text)
		_main.get_tree().quit(1)
		return

	var camera := Camera2D.new()
	camera.name = "Pass19CargoUpgradeCaptureCamera"
	...
	var output_path := "%s/%s_pass_19_cargo_upgrade.png" % [capture_dir, _safe_filename(_main._world.map_id)]
	var image: Image = _main.get_viewport().get_texture().get_image()
	image.save_png(output_path)
	print("Saved Pass 19 cargo upgrade capture: %s" % ProjectSettings.globalize_path(output_path))
	_main.get_tree().quit()
...
func _bank_until_upgrade_affordable() -> PackedStringArray:
	...
	for salvage in _main._salvage_centers_for_full_collection():
		if _main._session_wallet() >= _cargo_upgrade_cost():
			break
		...
		if _main._held_salvage >= _main._held_salvage_capacity():
			_main._player.global_position = _main._world.get_extraction_center()
			if _main._player.has_method("reset_motion"):
				_main._player.reset_motion()
			_main._process(0.0)

	if _main._held_salvage > 0 and _main._session_wallet() < _cargo_upgrade_cost():
		_main._player.global_position = _main._world.get_extraction_center()
		if _main._player.has_method("reset_motion"):
			_main._player.reset_motion()
		_main._process(0.0)
