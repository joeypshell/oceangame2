...
const PANEL_ONLY_STATES := ["archive_result", "stabilizer_project_promise"]
const CAPTURE_SIZES := [
...
func _verify_hud_layout(touch_visible: bool) -> bool:
	...
	for rect in touch_rects:
		if cargo_rect.intersects(rect) or tool_rect.intersects(rect):
			_fail("landscape-mobile touch controls overlap cargo or active tool")
			return false
	return true
...
func _save_capture(capture_dir: String, filename: String, image: Image) -> bool:
	...
	if error != OK:
		...
		return false
	print("Saved Expansion 14 capture: %s" % ProjectSettings.globalize_path(output_path))
	return true
