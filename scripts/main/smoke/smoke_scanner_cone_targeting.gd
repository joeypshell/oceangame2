...
func _init() -> void:
	...
	world.targets = [
		_target("far_straight", Vector2(160.0, 0.0)),
		_target("near_off_axis", Vector2(64.0, 1.0)),
	]
	...
	if not _failures.is_empty():
		for failure in _failures:
			push_error("Scanner cone targeting smoke failed: %s" % failure)
		quit(1)
		return
	print("Scanner cone targeting smoke passed: range_tiles=6 half_angle=30 ahead=true behind=false off_axis=false out_of_range=false occluded=false rank=angle,distance,id cancel=turn,move,occlusion progress_reset=true.")
	quit(0)
