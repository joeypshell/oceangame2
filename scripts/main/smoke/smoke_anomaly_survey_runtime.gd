...
func _run() -> void:
	...
	_expect(bool(runtime.report().get("scanner_use_held", false)), "ordinary identification did not retain held scanner input")
	...
	runtime.update(origin, player, 1.0)
	_expect(bool(runtime.report().get("scanner_use_held", false)), "ordinary scanner input expired before release")
	_expect(runtime.report().get("targeting", {}).get("scan_subject_label") == "Surface boat", "held identification lost its target")
	...
	runtime.scanner_release(origin)
	_expect(not bool(runtime.report().get("scanner_use_held", true)), "ordinary scanner input remained held after release")
	...
	_expect(str(initial.get("state", "")) == "awaiting_activation", "survey advanced before held Space/USE activation")
	...
	_expect(activated.get("reason") == "activated", "held Space/USE did not activate the nearby survey")
	...
	_expect(released.get("reason") == "released", "Space/USE release did not cancel the active survey")
	_expect(is_zero_approx(float(runtime.report().get("interaction", {}).get("progress", -1.0))), "Space/USE release retained partial progress")
	_place_for_scan(target_world, player, target)
	...
	_expect(str(retry_wait.get("state", "")) == "awaiting_activation", "returning to a canceled survey did not require a new held Space/USE")
	...
	_expect(regional_clue.find(str(regional_target.get("clue_label", ""))) != -1 and regional_clue.find("Hold Space/USE to scan") != -1, "regional clue or held scan action did not use source text")
...
func _exercise_harmonic_return(runtime, progression, profile, world, player) -> Dictionary:
	...
	_place_for_scan(world, player, target)
	_expect(runtime.scanner_action(world, player).get("reason") == "activated", "held Space/USE did not activate the harmonic survey")
...
func _exercise_abyssal_return(runtime, _progression, profile, world, player) -> Dictionary:
	...
	_place_for_scan(world, player, target)
	_expect(runtime.scanner_action(world, player).get("reason") == "activated", "held Space/USE did not activate the abyssal survey")
	var partial: Dictionary = runtime.update(world, player, 1.0)
	var partial_progress := float(partial.get("survey", {}).get("progress", 0.0))
