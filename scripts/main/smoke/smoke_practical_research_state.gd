...
func _run() -> void:
	...
	_expect(ready_overlay.find(str(target.get("clue_label", ""))) != -1 and ready_overlay.find("Hold Space/USE to scan: Survey mineral trace") != -1, "resource clue or held scan action was not source-derived")
	_expect(runtime.scanner_action(world, player).get("reason") == "activated", "held Space/USE did not activate resource research")
