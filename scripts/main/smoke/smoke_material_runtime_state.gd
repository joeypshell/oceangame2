...
func _run() -> void:
	...
	_expect(day_one_ids.size() == 4, "day one did not select the exact Ti2 + Rubber1 + Coil1 set")
	...
	_expect(_selected_material_count(world, day_one_ids, ExpansionProfileState.RUBBER_MATERIAL_ID) == 1, "day one rubber guarantee failed")
	...
	_expect(day_two_ids.size() == 4 and day_two_ids != day_one_ids, "next day did not rotate to a different valid set")
