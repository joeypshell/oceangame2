...
func _run() -> void:
	...
	_expect(presentation.promise_text(world, profile) == "Western chasm | Search the lower-left rock loop", "established west-first network promise drifted")
	_expect(presentation.promise_text(world, profile, ABYSS_JOURNEY_ID) == "Abyssal shelf | Search east through the pressure basin", "pinned abyss lead did not change guidance")
	...
	_expect(str(west_commit.get("note", "")).find("Abyssal Shelf Relay") != -1, "one-fragment feedback did not name the remaining lead")
