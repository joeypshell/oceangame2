...
func _run() -> void:
	...
	_expect(str(lead.get("ecology_lead_label", "")) == "MICA FOUND A TRACE", "Mica's reaction did not explain what she noticed")
	...
	_expect(str(palette.get("discovery_prompt_text", "")).find("hold BOND") != -1, "Mica's prompt omitted the BOND handoff")
	_expect(_status_notes.has("MICA FOUND A LIVING TRACE | Follow her signal, then hold BOND"), "Mica's first reaction was not announced")
