...
func activate(target: Dictionary) -> Dictionary:
	...
	return {
		...
		"note": "Scanner active | Hold Space/USE and position",
	}
...
func update(target: Dictionary, delta: float) -> Dictionary:
	...
	if target_id != _active_target_id:
		return {
			...
			"progress": 0.0,
			"note": "Hold Space/USE to scan: %s" % _display_label(target),
		}
	...
	var result := {
		...
		"interaction_seconds": _required_seconds,
		"progress": progress,
		"note": "%s %d%% | Hold Space/USE" % [_display_label(target), int(floor(progress * 100.0))],
	}
