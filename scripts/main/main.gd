...
func _ready() -> void:
	...
	if capture_original_map:
		...
		selected_map_path = PRODUCTION_LEVEL_MAP_PATH
	elif capture_expansion_15_expedition_planning:
		selected_map_path = PRODUCTION_LEVEL_MAP_PATH
	elif capture_expansion_16_deeper_wreck:
		selected_map_path = PRODUCTION_LEVEL_MAP_PATH
		...
		selected_map_path = PRODUCTION_LEVEL_MAP_PATH
	elif smoke_expansion_16_deeper_wreck:
		selected_map_path = PRODUCTION_LEVEL_MAP_PATH
	...
	if smoke_expansion_15_expedition_planning:
		...
		return
	if smoke_expansion_16_deeper_wreck:
		await SmokeExpansion16DeeperWreckChecks.new(self)._smoke_expansion_16_deeper_wreck_and_quit()
		return
	...
	if _has_arg(user_args, engine_args, "--capture-greybox-screenshot"):
		...
		var capture := Expansion15ExpeditionPlanningCapture.new(self)
		await capture.capture_and_quit(EXPANSION_15_EXPEDITION_PLANNING_CAPTURE_DIR)
	elif capture_expansion_16_deeper_wreck:
		var capture := Expansion16DeeperWreckCapture.new(self)
		await capture.capture_and_quit(EXPANSION_16_DEEPER_WRECK_CAPTURE_DIR)
	elif capture_expansion_17_wreck_network:
...
func _load_playable_map(
	...
	if float(_review_checkpoint_report.get("review_oxygen_seconds", 0.0)) > 0.0:
	...
	if _review_checkpoint_report.get("review_start_tile", {}) is Dictionary:
