...
func on_map_loaded(world) -> void:
	...
	if source.is_empty() or not bool(source.get("pressure_zone", false)):
		return
...
func update(position: Vector2, has_capability: Callable, delta: float) -> Dictionary:
	...
	var required_capability_id := str(_zone.get("required_capability_id", "")).strip_edges()
	var has_protection := (
		required_capability_id.is_empty()
		or (has_capability.is_valid() and bool(has_capability.call(required_capability_id)))
	)
	...
	if has_protection:
		...
		if entered:
			...
			_protected_feedback_seconds = maxf(0.0, _protected_feedback_seconds - maxf(0.0, delta))
		_note = "Pressure suit active" if _protected_feedback_seconds > 0.0 else ""
		return report()
	...
	if _exposure_seconds <= grace_seconds:
		_drain_multiplier = 1.0
		_note = "%s | Retreat" % str(_zone.get("pressure_label", "Abyssal pressure"))
	else:
		var source_multiplier := maxf(1.0, float(_zone.get("unprotected_oxygen_drain_multiplier", 1.0)))
		var critical_seconds := minf(frame_seconds, maxf(0.0, _exposure_seconds - maxf(previous_exposure, grace_seconds)))
		_drain_multiplier = (
			((frame_seconds - critical_seconds) + critical_seconds * source_multiplier) / frame_seconds
			if frame_seconds > 0.0
			else source_multiplier
		)
		_note = "Pressure critical | Oxygen x%d" % int(source_multiplier)
	return report()
