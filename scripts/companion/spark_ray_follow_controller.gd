...
const NEAR_EXIT_DISTANCE := 82.0
const FOLLOW_DISTANCE := 210.0
const SEPARATED_DISTANCE := 480.0
const WORRIED_SECONDS := 0.65
const PATH_REPLAN_SECONDS := 0.22
const WAYPOINT_REACHED_DISTANCE := 13.0
const PATH_LOOKAHEAD_POINTS := 12
const GATE_SAMPLE_DISTANCE := 8.0

const FOLLOW_SPEED := 175.0
const CATCH_UP_SPEED := 245.0
const RECOVERY_SPEED := 280.0
...
func set_external_control_active(active: bool) -> void:
	_external_control_active = active
	_path.clear()
	_path_index = 0
	_replan_seconds = 0.0
	if active:
		_state = STATE_EXTERNAL_CONTROL
	else:
		_state = STATE_NEAR
		_separated_seconds = 0.0
...
func step(
	...
	if _external_control_active:
		return _movement_result(Vector2.ZERO, 0.0, current_position.distance_to(player_position))
	...
	if _replan_seconds <= 0.0 or _path_index >= _path.size():
		_plan_path(current_position, follow_target, world, position_allowed)
	...
	if waypoint == current_position:
		return _movement_result(Vector2.ZERO, 0.0, distance)
	var speed := _speed_for_state(_state)
...
func _distance_state(distance: float) -> String:
	if _state == STATE_NEAR and distance <= NEAR_EXIT_DISTANCE:
		return STATE_NEAR
...
func _plan_path(start: Vector2, target: Vector2, world, position_allowed: Callable) -> void:
	...
	if not _path_blocked_by_gate and not _path.is_empty():
		var last_point := _path.back() as Vector2
		if last_point.distance_to(target) > WAYPOINT_REACHED_DISTANCE and _has_clear_allowed_segment(last_point, target, world, position_allowed):
			_path.append(target)
...
func _simplify_path(start: Vector2, source_path: Array, world, position_allowed: Callable) -> Array:
	...
	while cursor < source_path.size():
		...
		for candidate_index in range(limit, cursor - 1, -1):
			var candidate := source_path[candidate_index] as Vector2
			if _has_clear_allowed_segment(anchor, candidate, world, position_allowed):
...
func _has_clear_allowed_segment(start: Vector2, target: Vector2, world, position_allowed: Callable) -> bool:
	...
	var sample_count := maxi(1, int(ceil(distance / GATE_SAMPLE_DISTANCE)))
	for index in range(sample_count + 1):
		var sample := start.lerp(target, float(index) / float(sample_count))
		if not _position_allowed(sample, position_allowed):
			return false
	return true
