...
- Open: #82 prevent baseline acceptance tooling from copying ignored `.import` sidecars
...
- Open: #79 verify the public web preview after the player sprite pass
- Open: #78 decide player sprite baseline acceptance after implementation and review
- Open: #77 implement the controlled player sprite pass
- Open: #76 add or confirm a player-focused visual capture path for CVR02 review
...
- Open: #75 reconcile the `production_slice_01` accepted baseline with the current six-view capture set; created because #71 found broad pre-existing slice-01 baseline drift outside the prop pass
...
- `docs/current/PROP_SPRITE_BASELINE_DECISION.md` records #71: the prop sprites are approved for the current prototype, accepted baselines were updated for slices 02-04, and slice 01 baseline reconciliation is split into #75.
- `docs/current/BACKLOG_REFRESH_2026_07_06.md` records the current recommended issue order after #74.
...
- The second controlled visual-revision target is the player placeholder. It should add a named `assets/player/player_diver_01.png` asset and update `scenes/player/Player.tscn` without changing the player collision shape, movement controller, camera behavior, or gameplay logic.
...
1. Do #82 so baseline acceptance stops copying ignored `.import` sidecars.
2. Do #75 before relying on `production_slice_01` for clean controlled visual-revision diffs.
3. Do #76, then #77, #78, and #79 for the Controlled Visual Revision 02 player/diver sprite pass.
4. Do #80 and #81 to harden asset-path and public-preview verification.
5. Do #83 to capture the controlled visual-revision workflow as a checklist.
6. Do #84 only after the player pass is reviewed or deliberately deferred.
7. Keep #52/#53 as optional post-baseline slice-03 improvement issues if the accepted slice-03 reference needs intentional camera or source cleanup.
