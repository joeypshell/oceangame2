...
Expansion 11 is active under milestone #37. The source-state contract and focused validators are complete, and `dive_light_1` now resolves only from its durable profile project/capability pair. The old 900-score session purchase plus `L`/`LAMP` affordance are retired; full-level project/survey authoring, integrated journey proof, visual review, Web verification, and the player gate remain.
