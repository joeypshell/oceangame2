...
- Normal preview uses draft named sprite assets for salvage and hazard props, with procedural fallback if a sprite cannot be loaded.
- The first controlled visual-revision target landed as #70. It adds `assets/props/` sprites generated by `tools/generate_prop_sprites.py`, keeps map data, collision, behavior, debug markers, and accepted baselines unchanged, and leaves replacement baseline acceptance for review.
...
1. Review the #70 sprite-prop captures and `production_slice_*_visual_baseline_review.png` sheets before accepting any replacement baselines.
2. If the prop pass is accepted, create a small baseline-acceptance issue rather than folding acceptance into unrelated work.
