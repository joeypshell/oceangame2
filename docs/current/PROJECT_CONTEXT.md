...
- Open active batch: #113-#119 cover oxygen pressure baseline acceptance, salvage value tiers, one source-authored risk/reward placement in `production_slice_01`, route validation, baseline review, and public Web preview verification.
...
- `production_slice_01` marks `salvage_lower_loop` as the current `valuable` route-choice payoff target.
...
- `--smoke-route-choice` drives the player through the default slice route-choice probe by swimming from the boat entry to the lower-loop salvage target, collecting it, returning to extraction, and reporting the target/collection/return state.
...
- Salvage map data may include optional `tier` values. Missing tiers default conceptually to `common`; the current supported tiers are `common` and `valuable`.
...
1. Resolve #113 to record the oxygen pressure baseline decision after #112.
2. Resolve #114-#119 as a small batch: salvage value tiers, a distinct high-value salvage marker, one source-authored risk/reward placement in `production_slice_01`, route validation, baseline review, and public Web preview verification.
