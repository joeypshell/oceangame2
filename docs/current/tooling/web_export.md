...
The check fails if the web preview logs missing texture warnings such as `Unable to open texture asset`, `Unable to create cave TileSet`, `SCRIPT ERROR`, `ERROR:`, failed resource requests, a missing Godot canvas, a framing/readability mismatch between 1280x720 and 1920x1080 browser viewports, a touch-enabled 844x390 canvas that is not top anchored or does not cover the visual viewport, or an external `build_info.json` whose `git_sha` does not match the expected commit. Omit `--expected-sha` when checking an older export that does not include external build metadata.

The generated Web preset injects a small Head Include that fixes the page and canvas to the top of the browser viewport and uses `100dvh` with a `100vh` fallback. This keeps iPhone Safari browser chrome changes from vertically offsetting the game without hand-editing generated export files.
