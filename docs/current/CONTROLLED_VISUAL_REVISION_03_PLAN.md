...
Implementation status: #85 added the `boat_spawn_01.png` asset, generator, review sheet, and renderer integration while leaving source maps, player spawn, extraction semantics, collision, movement, gameplay logic, and accepted baselines unchanged. #86 accepted the boat sprite as the current prototype baseline and refreshed the accepted slice-01 visual baseline. #87 verified the public Pages preview after deployment and confirmed the boat sprite asset loads in browser with matching build metadata.
