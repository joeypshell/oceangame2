...
Implemented in issue #61 as `maps/production_slice_04.greybox.json`, generated by `tools/create_production_slice_04_map.py`.

The implementation preserves the selected lower-left loop topology, seals the focused crop edges, reauthors salvage/hazard objects in JSON, adds five camera tests, and uses an in-water `spawn + base` relay extraction plan near the east-side connector.
...
Evaluate #62 as a separate scoped pass. Do not change the default preview map or accept a slice-04 visual baseline as part of the implementation issue.
