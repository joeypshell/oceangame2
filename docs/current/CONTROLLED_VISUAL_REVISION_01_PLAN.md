...
- `assets/props/`
- `tools/generate_prop_sprites.py`
...
#70 adds draft named sprites for `crate`, `wreck_fragment`, `relic`, `mine`, and `jellyfish` under `assets/props/`, generated by `tools/generate_prop_sprites.py`. Runtime prop rendering now tries the sprite for the JSON `kind` first and falls back to the original procedural prop drawing if the sprite cannot be loaded.

#71 accepts those sprites as the current prototype prop-art baseline and updates accepted visual baselines for slices 02, 03, and 04. `production_slice_01` is intentionally left for #75 because its review sheet includes pre-existing broad accepted-baseline drift beyond the prop pass.
