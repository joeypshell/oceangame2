...
Follow-up implementation issue #50 was created from this decision. The implementation scope stayed limited to source generation, validation, captures, and documentation; no new gameplay systems were added as part of the slice selection work.

## Implementation Status

Implemented in issue #50 as `maps/production_slice_03.greybox.json`, generated by `tools/create_production_slice_03_map.py`.

The implementation preserves the selected high-fidelity sketch topology, seals the focused crop edges, fills unreachable pockets from the east-side relay spawn, reauthors salvage/hazard objects in JSON, adds five camera tests, and uses an in-water `spawn + base` relay extraction plan.
