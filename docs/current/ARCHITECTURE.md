...
- Expansion 15 is planning-only until its issue batch lands. The selected boundary adds one focused session/day-scoped selected-lead owner and a stateless resolver; `PrimaryDiveObjective`, `NextDiveObjectivePrompt`, `ProgressionProjectTracker`, `RegionalJourneyPresentation`, `ExpeditionDayDebrief`, profile state, and production source remain authoritative for their existing facts.
