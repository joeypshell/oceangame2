...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`; the latest completed implementation contract and decision are `docs/current/OCEANGAME_EXPANSION_04_PLAN.md` and `docs/current/OCEANGAME_EXPANSION_04_CLOSEOUT.md`. Expansion 04 closed with GO after focused visual review and exact-SHA Web verification. Expansion 05 issues #748-#757 are active under `docs/current/OCEANGAME_EXPANSION_05_PLAN.md`, with state/source ownership locked in the matching Expansion 05 contracts. Earlier controlled gameplay and visual passes remain implementation evidence, not the whole product direction.
