...
- `scripts/main/main.gd`
  ...
  - Uses `scripts/main/expedition_day_debrief.gd` for voluntary or forced night resolution, compact day totals, unbanked-state cleanup, the one-action salvage-cutter project, and the next-day reset. `daily_condition_presentation.gd` appends the next-day forecast derived by `daily_condition_state.gd`.
  ...
  - Keeps durable capabilities, committed discoveries including `propulsion_fins_blueprint`, banked typed materials, and exact-once project transactions in versioned `scripts/main/expansion_profile_state.gd`; schema v3 migrates valid v1 scanner/discovery and v2 cutter/material profiles. `scripts/main/material_project_runtime.gd` derives the authored project catalog and can select an eligible `dive_light_1_project` without inventing prerequisites from source-array order; `current_stabilizer` remains a later optional advanced-current project. `scripts/main/progression_project_tracker.gd` presents the active fins recipe with separate banked/held counts. Session wallet, held cargo, candidate depletion, oxygen, daylight, and in-progress interactions are not profile-persistent.
...
- `expedition_day_state.gd` caches each day's material selection. `material_candidate_selector.gd` applies a source-authored researched subset only for a fresh selection, while `material_runtime_controller.gd` exposes the matching compact habitat lead and preserves independent Ti2, Rubber1, and Coil1 pool guarantees, cargo, and boat banking.
...
- `--capture-upgrade-chest` provides desktop/mobile focused reviews for the fins project tracker and post-fins relay prompt without replacing accepted baselines.
