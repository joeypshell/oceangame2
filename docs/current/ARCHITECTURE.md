...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`. Expansions 13-17 are complete with owner GO. Planning gate #1190 selects Expansion 18 Transfer Hub Interior Expedition, but no implementation milestone or runtime is active. Its future contract must preserve one continuous sortie across a physically reached exceptional interior without turning historical connector support into normal traversal or adding transition state directly to `main.gd`.
...
- Expansion 18 is planning-only in `OCEANGAME_EXPANSION_18_PLAN.md`. Existing `WorldConnectorController` proves source-authored loading, but the historical connector contract resets local expedition state and is not the selected interior contract. A future focused transition owner must preserve live oxygen, daylight, health, cargo, pending recovery, and anti-duplication state across one paired entry/return while the canonical boat remains the only bank, commit, refill, night, and completion owner.
