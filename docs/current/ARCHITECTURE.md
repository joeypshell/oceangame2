...
- `scripts/main/main.gd`
  ...
  - Keeps durable capabilities, committed discoveries, banked typed materials, and exact-once project transactions in versioned `scripts/main/expansion_profile_state.gd`; schema v3 migrates valid v1 scanner/discovery and v2 cutter/material profiles. `scripts/main/material_project_runtime.gd` derives the source-ordered cutter-to-stabilizer catalog, one-project-per-debrief readiness, and compact text. Session wallet, held cargo, candidate depletion, oxygen, daylight, and in-progress interactions are not profile-persistent.
...
- Expansion 06 source/state ownership is locked in `OCEANGAME_EXPANSION_06_SOURCE_CONTRACT.md` and `OCEANGAME_EXPANSION_06_STATE_CONTRACT.md`: one immutable deep-cache territorial-lunge encounter, one durable non-circular shock-prod project, focused day-local hostile state, and player health separate from oxygen. The eel cache declares both its required capability and guarding hostile; collection is blocked until the shock prod exists and the eel is defeated for that day.
- `--smoke-expansion-06-combat-foundation` protects the unarmed cache lock, armed-but-active guard lock, contact damage/invulnerability, continuing oxygen/daylight pressure, exact project build/reload, three-hit no-reward victory, guarded cache collection/banking, connector/day state, combat cleanup, and failure resets.
...
- `expedition_day_state.gd` caches each day's material selection. `material_candidate_selector.gd` applies a source-authored researched subset only for a fresh selection, while `material_runtime_controller.gd` exposes the matching compact habitat lead and preserves Ti2+Coil1 yield, cargo, and boat banking.
