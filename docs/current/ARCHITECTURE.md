...
- `scripts/main/main.gd`
  ...
  - Keeps durable capabilities, committed discoveries including `propulsion_fins_blueprint`, banked typed materials, and exact-once project transactions in versioned `scripts/main/expansion_profile_state.gd`; schema v3 migrates valid v1 scanner/discovery and v2 cutter/material profiles. `scripts/main/material_project_runtime.gd` derives the blueprint-gated fins-to-cutter-to-stabilizer-to-weapon catalog and stage-aware guidance; `scripts/main/progression_project_tracker.gd` presents the active fins recipe with separate banked/held counts. Session wallet, held cargo, candidate depletion, oxygen, daylight, and in-progress interactions are not profile-persistent.
...
- The progression-gate lane adds `lower_left_loop_current`, a source-authored soft-push current gate overlapping the lower-left connector. It pushes the diver right until durable recipe-built `propulsion_fins` is present, while oxygen continues draining. All authored current gates render normal-world flow arrows; after fins, the same location presents the `E` relay prompt. It is protected by `--smoke-current-gate`.
...
- Expansion 05 adds `upper_right_mineral_trace_survey` beyond that gate. The source record owns its resource kind, scanner requirement, timing, clue/finding labels, durable discovery id, boat commit destination, and researched material-pool link.
...
- The progression-reward lane uses reachable `lower_loop_upgrade_chest` as the source-authored `propulsion_fins_blueprint`; recovery enters durable discovery knowledge without cargo or a second inventory. `--smoke-upgrade-chest` protects the complete blueprint-to-recipe-to-relay-to-scanner journey through validated route waypoints.
