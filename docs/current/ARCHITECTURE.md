...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`; the latest completed milestone decision is `docs/current/OCEANGAME_EXPANSION_09_CLOSEOUT.md`. Expansion 09 generated, reviewed, and promoted one contiguous full production level at exact Web build `e825c88`. Earlier controlled gameplay and visual passes remain implementation evidence, not the whole product direction.
...
- `scripts/main/main.gd`
  ...
  - Uses `scripts/main/session_progression.gd` for the session-only wallet and legacy oxygen, cargo, and light upgrades. Propulsion fins now use the durable material-project/profile path.
  ...
  - Keeps durable capabilities, committed discoveries including `propulsion_fins_blueprint`, banked typed materials, and exact-once project transactions in versioned `scripts/main/expansion_profile_state.gd`; schema v3 migrates valid v1 scanner/discovery and v2 cutter/material profiles. `scripts/main/material_project_runtime.gd` derives the mandatory fins-to-cutter-to-shock-prod-to-capacitor catalog, with `current_stabilizer` retained as a later optional advanced-current project; `scripts/main/progression_project_tracker.gd` presents the active fins recipe with separate banked/held counts. Session wallet, held cargo, candidate depletion, oxygen, daylight, and in-progress interactions are not profile-persistent.
