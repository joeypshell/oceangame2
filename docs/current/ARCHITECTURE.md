...
- `scripts/main/main.gd`
  ...
  - Delegates transient Spark Ray commands and riding to `scripts/companion/companion_control_runtime.gd`: `B/BOND` toggles bounded global slow time, desktop `1`-`3` directly activates the matching contextual row, and touch retains sequential `BOND`, `TOOL`, and `USE`; mounted movement stays collision- and gate-checked through the companion body while the player position remains authoritative for camera, oxygen, daylight, health, pressure, darkness, cargo, and failure. The mounted creature-action HUD replaces, then restores, the separate diver-tool HUD without persisting control state.
...
- one bounded command presentation intended to affect the whole simulation;
  #1336 owns verification/correction of effective hostile timing while open
