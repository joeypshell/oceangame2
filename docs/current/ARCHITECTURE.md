...
- `scripts/main/main.gd`
  ...
  - Delegates transient Spark Ray commands and riding to `scripts/companion/companion_control_runtime.gd`: Shift/BOND owns bounded global slow time and contextual commands; mounted movement stays collision- and gate-checked through the companion body while the player position remains authoritative for camera, oxygen, daylight, health, pressure, darkness, cargo, and failure. The mounted creature-action HUD replaces, then restores, the separate diver-tool HUD without persisting control state.
