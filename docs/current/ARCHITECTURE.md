...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`. Expansion 13 remains at owner HOLD after exact runtime `89371d6`. #1000-#1004 establish the active-tool correction plus compact controls and individually named titanium/bundled-rubber/spring-coil material presentation; the sealed-wreck progression reward and integrated evidence remain in #1005-#1009. #969 remains the owner replay gate and no next expansion is selected.
...
- Expansion 13 reuses that flow for `southeast_wreck_archive_survey`. The source-authored regional journey links the committed abyssal discovery to a broad wreck lead; `SurveyDependencyState` requires the cutter-cleared recorder before scanning, `ExpansionProfileState` persists source-marked recorder clearance and the committed discovery, and `RegionalJourneyPresentation` derives compact cutter, scan, pending-return, and result guidance without exact-route handholding.
