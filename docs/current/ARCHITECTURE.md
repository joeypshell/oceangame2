...
- Expansion 06 source/state ownership is locked in `OCEANGAME_EXPANSION_06_SOURCE_CONTRACT.md` and `OCEANGAME_EXPANSION_06_STATE_CONTRACT.md`: one immutable deep-cache territorial-lunge encounter, one durable non-circular shock-prod project, focused day-local hostile state, and player health separate from oxygen. The bounded runtime, integrated journey, visual decision, and exact-SHA Web verification are complete with GO.
- `--smoke-expansion-06-combat-foundation` protects unarmed retreat, contact damage/invulnerability, continuing oxygen/daylight pressure, exact project build/reload, three-hit no-reward victory, cargo/banking compatibility, connector/day state, combat cleanup, and failure resets.
