...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`. Expansions 13-16 are complete with owner GO; Expansion 16's exact reviewed runtime is `05b482e`. Its source-authored far-west route composes focused oxygen-consumption, project/profile, cutter, scanner, pending-discovery, boat-commit, and HUD owners without moving those rules into the map or `main.gd`. Expansion 17 Wreck Network Triangulation is active in milestone #43; its contract is locked while runtime implementation remains pending.
...
- `scanner_cone_targeting.gd` requires an explicit held `Q/SCAN`, a target ahead within the fixed cone and range, and unobstructed terrain line of sight. Ranking is deterministic; release, tool switch, moving away, turning away, occlusion, failure, or reset cancels progression. Ordinary subjects provide reward-free target-local identification. Scanner presentation owns only the transient held cone, bracket, miss, readout, and progress feedback.
...
- Expansion 17 ownership is locked in `OCEANGAME_EXPANSION_17_SOURCE_STATE_CONTRACT.md`. It reuses `ExpansionProfileState`, `ExpeditionDiscoveryState`, `ExpeditionLeadResolver`, `ExpeditionPlanState`, scanner owners, and night debrief/presentation, with one focused investigation state deriving the two-fragment aggregate and explicit no-cost night analysis. Source owns physical artifacts and relationships; mutable selection, pending state, committed fragments, analysis readiness, and UI visibility are not authored in JSON or added directly to `main.gd`.
