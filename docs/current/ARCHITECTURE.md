...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`. Expansion 13 is complete with owner GO at `ede39d1`. Expansion 14 reached technical PASS at `f2f2750`, but owner review recorded HOLD for combat readability/control, scanner interaction, relay identity, and passive-equipment presentation. The bounded correction contract is `docs/current/OCEANGAME_EXPANSION_14_OWNER_HOLD_CORRECTION_PLAN.md`, implemented only through #1069-#1077 before returning to #1040. Existing progression, cargo, failure, boat-commit, topology, and active-tool ownership remain authoritative; no closeout or later expansion is selected.
...
- `scanner_cone_targeting.gd` requires an explicit `Q/SCAN`, a target ahead within the fixed cone and range, and unobstructed terrain line of sight. Ranking is deterministic; moving, turning away, occlusion, failure, or reset cancels progress. Scanner presentation owns only the transient cone, bracket, miss, and progress feedback.
...
- `held_cargo_hud.gd` is a read-only projection of current-sortie materials and valuable cargo plus used/free capacity. It owns no cargo or profile state and remains visually separate from `active_tool_hud.gd` on desktop and landscape mobile.
- `--smoke-expansion-14-archive-current-return` plus the focused runtime-owner and held-cargo HUD scripts protect the complete archive/project/current/core/survey/boat sequence and legacy compatibility. `--capture-expansion-14-archive-current-return` renders eight deterministic states at both review sizes without mutating map source or accepting focused captures.
...
- `shock_prod_presentation.gd` is a read-only player-local effect: each real discharge consumes the controller result and authoritative 72px range to show a brief forward semicircle, electrical bolt, and hit impact without owning targeting, damage, or cooldown state.
