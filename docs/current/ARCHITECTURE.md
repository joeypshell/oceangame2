...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`. Expansions 13-15 are complete with owner GO at exact runtimes `ede39d1`, `1f148eb`, and `23f4172`. Expansion 15 composes existing source journeys, daily conditions, profile/project facts, and night/day state into one session-scoped choice without duplicating those owners. Expansion 16 Deeper Wreck Oxygen Return is selected in planning only; current runtime architecture is unchanged.
...
- Expansion 16 is currently a plan, not an implemented owner. Its proposed boundary keeps mutable oxygen in `SortieState`, route-local overlap/multiplier/readability in one focused oxygen-pressure controller, durable rebreather/discovery state in `ExpansionProfileState`, exact materials/night construction in `MaterialProjectRuntime`, and route/zone/target/survey relationships in a future source helper. Do not add these rules directly to `main.gd`.
