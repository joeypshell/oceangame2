...
- `maps/full_cave_sketch_01.greybox.json`
  ...
  - Source topology for the planned separately generated `maps/production_level_01.greybox.json` candidate.
...
- `maps/production_level_01.greybox.json` (planned, not implemented)
  - Will derive from the complete `158 x 161` full-sketch topology, not from stitched production-slice JSON.
  - Will reuse the current slice-01 gameplay overlay through a source-owned `+58, +0` coordinate transform while omitting crop seals and connector metadata.
  - Will remain a selectable candidate until full-map clearance, parity, route, camera, performance, visual, player, and Web review approve default promotion.
- `maps/production_slice_01.greybox.json`
...
- `maps/production_slice_02.greybox.json`
  ...
  - Not the default preview map; its former anomaly survey moved to the contiguous east pocket in `production_slice_01`.
...
- `scripts/main/main.gd`
  ...
  - Delegates visual capture flags to `scripts/main/capture_controller.gd` and focused capture helpers, including camera tests, route outcome, timed/pry/cutter salvage, controlled-pass states, progression/connectors, anomaly survey, expedition day, Expansion 03 material/project, Expansion 04 current-pocket, Expansion 05 practical research, Expansion 06 combat, Expansion 07 biological progression, and Expansion 08 daily-condition review states.
  - Delegates smoke checks to `scripts/main/smoke/`, including safe/deep route comparison, timed/pry/cutter salvage, Expansion 03-08 journeys, focused combat/biological/current/profile/condition state, hazard and oxygen pressure, cargo/scoring, route/result/facing checks, and production-slice routes.
