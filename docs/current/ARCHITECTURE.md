...
- Expansion 15 ownership is locked in `OCEANGAME_EXPANSION_15_SOURCE_STATE_CONTRACT.md`: one focused session-scoped selected-lead owner, a stateless resolver, debrief-only choice presentation, and context-routed existing inputs. `PrimaryDiveObjective`, `NextDiveObjectivePrompt`, `ProgressionProjectTracker`, `RegionalJourneyPresentation`, `ExpeditionDayDebrief`, profile state, and production source remain authoritative for their existing facts.
