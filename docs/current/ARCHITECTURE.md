...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`; the latest completed implementation contract and decision are `docs/current/OCEANGAME_EXPANSION_05_PLAN.md` and `docs/current/OCEANGAME_EXPANSION_05_CLOSEOUT.md`. Expansion 05 closed with GO after focused visual review and exact-SHA Web verification. Expansion 06 Combat Foundation is active through issues #768-#777, with its bounded plan and ownership contracts under `docs/current/`. Earlier controlled gameplay and visual passes remain implementation evidence, not the whole product direction.
...
- `scripts/main/main.gd`
  ...
  - Delegates player combat health, hit invulnerability, connector preservation, canonical-boat refill, and zero-health reporting to `scripts/main/player_health_state.gd`. Health remains separate from oxygen; hostile attack behavior is added only by the subsequent Expansion 06 runtime issue.
