...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`. Expansion 13 remains at owner HOLD after exact runtime `89371d6`. #1000-#1005 establish the active-tool correction, compact controls, named material presentation, and source-authored sealed-wreck navigation-data reward; runtime reward commitment and integrated evidence remain in #1006-#1009. #969 remains the owner replay gate and no next expansion is selected.
...
- Expansion 13 reuses that flow for `southeast_wreck_archive_survey`. The pressure suit remains transitively grounded in the abyssal discovery, while the regional journey now directly requires `southeast_wreck_navigation_data` authored on the earlier sealed-wreck cutter target and committed at `surface_boat_entry`; #1006 owns runtime pending/commit consumption. `SurveyDependencyState` still requires the cutter-cleared recorder before scanning.
