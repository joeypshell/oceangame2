...
- Expansion 17 remains planning-only. The proposed boundary reuses `ExpansionProfileState`, `ExpeditionDiscoveryState`, `ExpeditionLeadResolver`, `ExpeditionPlanState`, scanner owners, and night debrief/presentation, with one focused investigation resolver deriving the two-fragment aggregate. Source owns physical artifacts and relationships; mutable selection, pending state, committed fragments, analysis readiness, and UI visibility must not be authored in JSON or added directly to `main.gd`.
