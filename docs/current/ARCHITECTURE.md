...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`. Expansions 13 and 14 are complete with owner GO at exact runtimes `ede39d1` and `1f148eb`. Expansion 14's bounded correction preserves existing progression, cargo, failure, boat-commit, topology, and active-tool ownership while adding focused scanner, relay, combat-feedback, passive-equipment, and failure-lock presentation. No later expansion is selected.
