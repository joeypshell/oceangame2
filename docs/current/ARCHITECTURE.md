...
- Living Expedition 03 ownership is locked in `LIVING_EXPEDITION_03_SOURCE_STATE_CONTRACT.md`. Current map/catalog source will own one Mica relationship linked by id to the existing southwest bloom and moving patrol without copying geometry. A focused transient observation owner will bridge existing Reveal Trace and Scanner identification to exact-once canonical-boat memory commitment; `CompanionProfileState` keeps schema v2 and remains the durable memory/adaptation owner. Existing moving-hazard owners retain phase, position, contact, and reset authority while focused Drift Lens owners may read snapshots for temporary path/direction projection only.
