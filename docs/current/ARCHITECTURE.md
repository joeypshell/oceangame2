...
The long-range product roadmap is `docs/planning/OCEANGAME_PHASE_2_ROADMAP.md`; the latest completed implementation contract and decision are `docs/current/OCEANGAME_EXPANSION_05_PLAN.md` and `docs/current/OCEANGAME_EXPANSION_05_CLOSEOUT.md`. Expansion 05 closed with GO after focused visual review and exact-SHA Web verification. Expansion 06 Combat Foundation is selected for the next planning cycle, but no implementation batch exists yet. Earlier controlled gameplay and visual passes remain implementation evidence, not the whole product direction.
