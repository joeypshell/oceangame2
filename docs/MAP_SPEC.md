...
Expansion-specific survey dependencies remain in their source/state contracts. Survey metadata belongs only in `survey_targets` and may not author runtime/profile state; cross-map relationships are checked with `python tools/audit_progression_graph.py`. Living Expedition creature records and immutable catalog relationships follow `docs/current/LIVING_EXPEDITION_01_MAP_SCHEMA.md` plus the current Living Expedition source/state contracts. LE04 permits one top-level `companion_hostile_responses` record linking the existing eel, guarded cache, defeat-only harvest, and one catalog-backed Guardian-Pulse response. LE05 adds one optional Cutter-aid Silt Hound rescue and one `material_excavation_review` context leading to a normal optional material candidate. LE06 adds one optional `regional_creature_journeys` record linked to one guaranteed `passive_wildlife_groups` school, one `creature_nurseries` destination, one non-damaging `ecological_pressures` record, and two catalog-backed `regional_journey_action` contexts as defined in `LIVING_EXPEDITION_06_SOURCE_STATE_CONTRACT.md`. These chains cannot grant rewards, own mutable state, satisfy mandatory material guarantees, or become required project/access dependencies. Map JSON never owns mutable individual, selection, memory, bond, adaptation, mounted, reveal, hostile, cooldown, journey, commitment, or restoration state.
