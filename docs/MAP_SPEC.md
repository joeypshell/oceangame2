...
Darkness/light metadata is defined in `docs/current/DEPTH_DARKNESS_LIGHT_GATE_CONTRACT.md`. Expansion 11 keeps visibility zones visual-only and uses `required_light_capability_id` only on `signal_reef_deep_harmonic_survey`, alongside the scanner's existing `required_capability_id`; the light blocks survey progress, never water, collision, or target rendering.
...
Expansion 08 daily-condition metadata follows `docs/current/OCEANGAME_EXPANSION_08_SOURCE_CONTRACT.md`: one optional even-day condition may link one optional bonus material pool and one moving hazard, but never terrain or mandatory progression.
Non-salvage surveys use top-level `survey_targets`: anomaly rules live in `docs/current/OCEANGAME_EXPANSION_01_SURVEY_SOURCE_CONTRACT.md`, the Expansion 05 resource target lives in `docs/current/OCEANGAME_EXPANSION_05_SOURCE_CONTRACT.md`, and regional targets live in the Expansion 10 and 11 source contracts. Survey metadata must not be placed on salvage entities or author runtime/profile state. Expansion 03 material candidates/projects follow `docs/current/OCEANGAME_EXPANSION_03_SOURCE_CONTRACT.md`; Expansion 11's `dive_light_1_project` follows `docs/current/OCEANGAME_EXPANSION_11_SOURCE_STATE_CONTRACT.md`. Cross-map relationships are checked with `python tools/audit_progression_graph.py`; only runtime purchase/scoring relationships absent from map data belong in `config/progression_contract.json`.
```json
{
...
- Keep ordinary progression geography contiguous in one authored map where practical. A mandatory current, darkness, oxygen, pressure, or tool gate should reveal another authored route/area rather than silently teleporting to a separate map.
