...
Darkness/light metadata is defined in `docs/current/DEPTH_DARKNESS_LIGHT_GATE_CONTRACT.md`. Expansion 11 keeps visibility zones visual-only and uses `required_light_capability_id` only on `signal_reef_deep_harmonic_survey`, alongside the scanner's existing `required_capability_id`; the light blocks survey progress, never water, collision, or target rendering. Expansion 12 pressure metadata is locked in `docs/current/OCEANGAME_EXPANSION_12_SOURCE_STATE_CONTRACT.md`: `pressure_zone`, level/label, durable capability, grace, drain multiplier, and route context belong on one marker, while `required_pressure_capability_id` belongs only on its contracted survey.
...
A `regional_journey` may use `payoff_target_id` only for valuable salvage inside its landmark whose `route_context` links back to that journey; it does not make the cargo profile-durable. Expansion 15 planning metadata follows `docs/current/OCEANGAME_EXPANSION_15_SOURCE_STATE_CONTRACT.md`: only `regional_journeys` and `daily_conditions` may add an optional nested `expedition_lead` with exact `lead_type`, `label`, `summary`, `active_guidance`, and `order` fields; the parent id remains the lead id, and the block may not author selection, readiness, rewards, runtime state, or duplicated source relationships.
```json
{
