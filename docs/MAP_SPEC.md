...
## Purpose
The map must be a data source, not a screenshot to imitate. The game scene renders from authored map data so the in-game result can match the plan. Target creature habitats, rescues, memories, equipment boundaries, and payoffs follow `docs/planning/CREATURE_MAP_PROGRESSION_SPEC.md`; this file does not claim their schema is implemented.
...
Chest/cache metadata is defined in `docs/current/LOCKED_CACHE_PROGRESSION_CONTRACT.md`. Use top-level `progression_containers`; a `blueprint` reward must link to exactly one material project's `required_discovery_id`, omit `reward_amount`, and reuse durable discovery state rather than adding inventory. An optional cutter `tool_target` discovery reward uses `reward_kind`, `reward_id`, three `reward_*_label` fields, and namespaced `reward_commit_map_id`, `reward_commit_map_path`, and `reward_commit_entry_id`; it must retain valuable cargo, commit at the source map's canonical boat, and never depend on the regional journey it unlocks.

Darkness/light metadata is defined in `docs/current/DEPTH_DARKNESS_LIGHT_GATE_CONTRACT.md`. Expansion 11 keeps visibility zones visual-only and uses `required_light_capability_id` only on its contracted survey. Expansion 12 pressure metadata is locked in its source/state contract. Expansion 16 uses one non-solid `oxygen_consumption_zone` marker with label, rebreather capability, grace, multiplier, and route context as defined in `docs/current/OCEANGAME_EXPANSION_16_SOURCE_STATE_CONTRACT.md`; the scanner survey does not duplicate the rebreather as an interaction gate.
...
Non-salvage surveys use top-level `survey_targets`; the focused physical-subject and reward block is defined in `docs/current/SCAN_SUBJECT_SOURCE_CONTRACT.md`. Existing surveys may omit that block, but any target using it must declare the complete subject id/kind, display label/description, presentation, anchor, and reward contract; only an artifact may reward a blueprint. Gameplay-relevant records without the block use deterministic informational fallback identity and never grant scanner rewards.
Expansion-specific survey dependencies remain in their source/state contracts. Survey metadata belongs only in `survey_targets` and may not author runtime/profile state; cross-map relationships are checked with `python tools/audit_progression_graph.py`. Living Expedition creature records and immutable catalog relationships follow `docs/current/LIVING_EXPEDITION_01_MAP_SCHEMA.md` plus the current Living Expedition source/state contracts. LE04 permits one top-level `companion_hostile_responses` record linking the existing eel, guarded cache, defeat-only harvest, and one catalog-backed Guardian-Pulse response; it may not copy state/geometry, grant rewards, expose harvests, or mutate profiles. LE05 adds one optional Cutter-aid Silt Hound rescue, one `material_excavation_review` context, and one normal `material_candidate` in a one-candidate `optional_bonus` pool. The candidate alone may declare `buried_deposit: true`, `required_companion_action_id: excavate`, its `companion_context_id`, and `presentation_kind: buried_mineral_mound`; the context owns the candidate and canonical boat ids while source provenance lists the rescue, habitat, context, candidate, pool, cameras, and no terrain changes. This chain cannot satisfy mandatory material guarantees or required project/access dependencies. Map JSON never owns mutable individual, selection, memory, bond, adaptation, mounted, reveal, hostile, or cooldown state.
A `regional_journey` may use `payoff_target_id` only for valuable salvage inside its landmark whose `route_context` links back to that journey; it does not make the cargo profile-durable. It may use one compact `approach_guidance` line for source-owned in-dive direction after its prerequisite is met and before its result discovery is complete. Expansion 15 planning metadata follows `docs/current/OCEANGAME_EXPANSION_15_SOURCE_STATE_CONTRACT.md`: only `regional_journeys` and `daily_conditions` may add an optional nested `expedition_lead` with exact `lead_type`, `label`, `summary`, `active_guidance`, and `order` fields; the parent id remains the lead id, and the block may not author selection, readiness, rewards, runtime state, or duplicated source relationships.
Expansion 17 may add optional top-level `wreck_network_investigations` records that normalize a prerequisite, exactly two fragment discovery ids, one night-analysis discovery, compact result/next-lead labels, and the canonical boat commit. Fragment surveys reference the aggregate with `investigation_id`; all selection, pending, completion, analysis-readiness, cost, reward, and profile state remains runtime-owned. The exact current contract is `docs/current/OCEANGAME_EXPANSION_17_SOURCE_STATE_CONTRACT.md`.
```json
{
...
- World connector metadata is supported only on marker zones. Connector rectangles must be in bounds, non-solid, reachable, and reference a committed destination map plus an existing destination `spawn` or `boat_spawn` entry id. Prompted connectors are exceptional entrances/interiors, not the default way to cross a capability gate.
- Current gate metadata is supported only on marker zones. Current rectangles must be in bounds, non-solid, reachable, use a supported direction, positive strength, and one lower_snake_case required upgrade/capability id. Standard currents must be crossed through normal movement after the requirement is owned; they must not require `E`/ACT.
- Keep ordinary progression geography contiguous in one authored map where practical. A mandatory current, darkness, oxygen, pressure, or tool gate should reveal another authored route/area rather than silently teleporting to a separate map; pressure markers must remain non-collision volumes and pass the focused crossing, retreat, oxygen-budget, and boat-return validator. A regional journey may reuse an earlier gate only when `required_discovery_id` resolves to that earlier journey's survey and an optional `tool_target_id` resolves to the single tool target that unlocks the journey survey.
