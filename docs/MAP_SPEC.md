...
Non-salvage surveys use top-level `survey_targets`: anomaly rules live in `docs/current/OCEANGAME_EXPANSION_01_SURVEY_SOURCE_CONTRACT.md`, the Expansion 05 resource target lives in `docs/current/OCEANGAME_EXPANSION_05_SOURCE_CONTRACT.md`, and regional targets live in the Expansion 10-13 source contracts. Expansion 13 permits one recorder tool target to reuse `salvage_cutter_project` while owning `unlocks_survey_target_id` plus `durable_clearance`; the survey must not duplicate that dependency. Survey metadata must not be placed on salvage entities or author runtime/profile state. Expansion 03 material candidates/projects follow `docs/current/OCEANGAME_EXPANSION_03_SOURCE_CONTRACT.md`; Expansion 11's light, Expansion 12's pressure suit, and Expansion 13's wreck chain each follow their current source/state contract. Cross-map relationships are checked with `python tools/audit_progression_graph.py`; only runtime purchase/scoring relationships absent from map data belong in `config/progression_contract.json`.
```json
{
