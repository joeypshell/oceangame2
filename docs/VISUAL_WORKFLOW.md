...
Target: replace procedural salvage and hazard prop drawings with named committed sprite assets, then compare the result against accepted production-slice baselines before accepting any replacement baseline. The draft prop sprites are generated by `tools/generate_prop_sprites.py` and reviewed in `references/asset_reviews/prop_sprites_01_review.png`.
