...
First controlled visual revision target: replace the original procedural salvage/hazard drawings with named sprite assets under #70. The current draft sprites are generated by `tools/generate_prop_sprites.py`, and the runtime selects them from existing JSON `kind` values with procedural fallback if an asset is missing.

| Asset | Size | Purpose | Status | Notes |
|---|---:|---|---|---|
| `assets/props/salvage_crate_01.png` | 32x32 | Collectible crate sprite | draft | Warm crate accent; replaces procedural `crate` salvage art in normal gameplay. |
| `assets/props/wreck_fragment_01.png` | 32x32 | Collectible wreck-fragment sprite | draft | Metal/rust accent; replaces procedural `wreck_fragment` salvage art in normal gameplay. |
| `assets/props/relic_01.png` | 32x32 | Collectible relic sprite | draft | Teal/gold accent; replaces procedural `relic` salvage art in normal gameplay. |
| `assets/props/mine_01.png` | 32x32 | Hazard mine sprite | draft | High-readability red hazard; replaces procedural `mine` hazard art in normal gameplay. |
| `assets/props/jellyfish_01.png` | 32x32 | Hazard jellyfish sprite | draft | Magenta warning-role hazard; replaces procedural `jellyfish` hazard art in normal gameplay. |
