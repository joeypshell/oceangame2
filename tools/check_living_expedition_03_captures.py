#!/usr/bin/env python3
"""Validate the generated Living Expedition 03 visual-evidence set."""
...
CAPTURE_DIR = ROOT / "visual_captures" / "living_expedition_03"
MANIFEST_PATH = CAPTURE_DIR / "capture_manifest.json"
PNG_SIGNATURE = b"\x89PNG\r\n\x1a\n"
EXPECTED_CHECKPOINT = "living_expedition_03_start"
...
def main() -> int:
    ...
    if manifest.get("review_checkpoint") != EXPECTED_CHECKPOINT:
        return fail("capture did not use the isolated LE03 checkpoint")
    ...
    if manifest.get("subject") != EXPECTED_SUBJECT:
        return fail("manifest does not identify the living migration subject")
    ...
    tracked = subprocess.run(
        ["git", "ls-files", "--", "visual_captures/living_expedition_03/*.import"],
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
    ...
    print(
        "PASS: Living Expedition 03 captures "
