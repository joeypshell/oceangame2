#!/usr/bin/env python3
"""Validate the generated Living Expedition 06 visual-evidence set."""
...
CAPTURE_DIR = ROOT / "visual_captures" / "living_expedition_06"
MANIFEST_PATH = CAPTURE_DIR / "capture_manifest.json"
PNG_SIGNATURE = b"\x89PNG\r\n\x1a\n"
EXPECTED_CHECKPOINTS = (
    "living_expedition_06_anchor_ready",
    "living_expedition_06_guardian_ready",
    "living_expedition_06_restored_nursery",
)
EXPECTED_SUBJECT = "Signal Reef adaptation branches and next-day nursery restoration"
EXPECTED_STATES = (
    "approach",
    "anchor_action",
    "guardian_action",
...
def main() -> int:
    ...
    if tuple(manifest.get("review_checkpoints", [])) != EXPECTED_CHECKPOINTS:
        return fail("capture did not use all three isolated LE06 checkpoints")
    ...
    if manifest.get("subject") != EXPECTED_SUBJECT:
        return fail("manifest does not identify the Signal Reef proof")
    ...
    tracked = subprocess.run(
        ["git", "ls-files", "--", "visual_captures/living_expedition_06"],
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
    ...
    tracked_files = sorted(line for line in tracked.stdout.splitlines() if line)
    if tracked_files:
        return fail(f"tracked generated evidence found: {tracked_files}")

    ignored = subprocess.run(
        ["git", "check-ignore", "--quiet", "visual_captures/living_expedition_06/capture_manifest.json"],
        cwd=ROOT,
        check=False,
    )
    if ignored.returncode != 0:
        return fail("Living Expedition 06 evidence directory is not ignored")

    print(
        "PASS: Living Expedition 06 captures "
